...
3. trend 브리핑은 OpenAlex와 OpenAI를 사용해 생성하되, 지난 7일 논문 안에서 연구 참고 가치가 높은 논문을 우선 선정한다.
...
3. trend는 지난 7일 논문만 다루되, 그 안에서 재현 가능성, 실험 밀도, 후속 연구 연결성이 높은 논문을 우선 다룬다.
...
3. 연구 참고 가치 기준으로 상위 후보를 고르고 OpenAI로 브리핑을 생성한다.
...
- `LLM과 추론`
  거대언어모델, 추론, 정렬, 파인튜닝 같은 주제
- `이미지·영상 이해`
  분류, 검출, 분할, 추적, 의료영상 같은 비전 이해 주제
- `멀티모달·에이전트`
  텍스트+이미지+행동을 함께 다루는 모델과 AI 에이전트
- `음성·오디오 AI`
  음성 인식, 음성 합성, 오디오 이해 관련 주제
- `검색·RAG`
  검색, 리트리벌, RAG, 재정렬 같은 정보찾기 주제
- `로봇·자율행동`
  로봇 제어, 이동, 조작, embodied AI 관련 주제
- `이미지·영상 생성`
  diffusion, 생성, 편집, 크리에이티브 AI 관련 주제
- `데이터·학습 방법`
  데이터셋, 합성 데이터, 학습 전략, 평가 데이터 관련 주제
- `모델 경량화·서빙`
  양자화, distillation, 추론 속도, 메모리, 배포 효율화 주제
- `기타 AI 연구`
  위 분류에 깔끔하게 안 들어가는 경우
